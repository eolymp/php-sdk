<?php
# Generated by the protocol buffer compiler.  DO NOT EDIT!
# source: eolymp/course/editor_service.proto

namespace Eolymp\Course\Editor;

use Google\Protobuf\Internal\GPBType;
use Google\Protobuf\Internal\RepeatedField;
use Google\Protobuf\Internal\GPBUtil;

/**
 * Generated from protobuf message <code>eolymp.course.Editor.State</code>
 */
class State extends \Google\Protobuf\Internal\Message
{
    /**
     * Generated from protobuf field <code>string runtime = 1;</code>
     */
    protected $runtime = '';
    /**
     * Generated from protobuf field <code>string source_code = 2;</code>
     */
    protected $source_code = '';
    /**
     * Generated from protobuf field <code>string input_data = 3;</code>
     */
    protected $input_data = '';
    protected $payload;

    /**
     * Constructor.
     *
     * @param array $data {
     *     Optional. Data for populating the Message object.
     *
     *     @type string $runtime
     *     @type string $source_code
     *     @type string $input_data
     *     @type \Eolymp\Atlas\Submission\Output $output
     *           OUTPUT: the files uploaded so far
     * }
     */
    public function __construct($data = NULL) {
        \GPBMetadata\Eolymp\Course\EditorService::initOnce();
        parent::__construct($data);
    }

    /**
     * Generated from protobuf field <code>string runtime = 1;</code>
     * @return string
     */
    public function getRuntime()
    {
        return $this->runtime;
    }

    /**
     * Generated from protobuf field <code>string runtime = 1;</code>
     * @param string $var
     * @return $this
     */
    public function setRuntime($var)
    {
        GPBUtil::checkString($var, True);
        $this->runtime = $var;

        return $this;
    }

    /**
     * Generated from protobuf field <code>string source_code = 2;</code>
     * @return string
     */
    public function getSourceCode()
    {
        return $this->source_code;
    }

    /**
     * Generated from protobuf field <code>string source_code = 2;</code>
     * @param string $var
     * @return $this
     */
    public function setSourceCode($var)
    {
        GPBUtil::checkString($var, True);
        $this->source_code = $var;

        return $this;
    }

    /**
     * Generated from protobuf field <code>string input_data = 3;</code>
     * @return string
     */
    public function getInputData()
    {
        return $this->input_data;
    }

    /**
     * Generated from protobuf field <code>string input_data = 3;</code>
     * @param string $var
     * @return $this
     */
    public function setInputData($var)
    {
        GPBUtil::checkString($var, True);
        $this->input_data = $var;

        return $this;
    }

    /**
     * OUTPUT: the files uploaded so far
     *
     * Generated from protobuf field <code>.eolymp.atlas.Submission.Output output = 11;</code>
     * @return \Eolymp\Atlas\Submission\Output|null
     */
    public function getOutput()
    {
        return $this->readOneof(11);
    }

    public function hasOutput()
    {
        return $this->hasOneof(11);
    }

    /**
     * OUTPUT: the files uploaded so far
     *
     * Generated from protobuf field <code>.eolymp.atlas.Submission.Output output = 11;</code>
     * @param \Eolymp\Atlas\Submission\Output $var
     * @return $this
     */
    public function setOutput($var)
    {
        GPBUtil::checkMessage($var, \Eolymp\Atlas\Submission\Output::class);
        $this->writeOneof(11, $var);

        return $this;
    }

    /**
     * @return string
     */
    public function getPayload()
    {
        return $this->whichOneof("payload");
    }

}

// Adding a class alias for backwards compatibility with the previous class name.
class_alias(State::class, \Eolymp\Course\Editor_State::class);

