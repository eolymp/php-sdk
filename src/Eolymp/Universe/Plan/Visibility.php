<?php
# Generated by the protocol buffer compiler.  DO NOT EDIT!
# source: eolymp/universe/plan.proto

namespace Eolymp\Universe\Plan;

use UnexpectedValueException;

/**
 * Protobuf type <code>eolymp.universe.Plan.Visibility</code>
 */
class Visibility
{
    /**
     * Generated from protobuf enum <code>UNKNOWN_VISIBILITY = 0;</code>
     */
    const UNKNOWN_VISIBILITY = 0;
    /**
     * offered to every space
     *
     * Generated from protobuf enum <code>PUBLIC = 1;</code>
     */
    const PBPUBLIC = 1;
    /**
     * offered on request, requires approval
     *
     * Generated from protobuf enum <code>PRIVATE = 2;</code>
     */
    const PBPRIVATE = 2;
    /**
     * offered only to spaces it is assigned to
     *
     * Generated from protobuf enum <code>INTERNAL = 3;</code>
     */
    const INTERNAL = 3;

    private static $valueToName = [
        self::UNKNOWN_VISIBILITY => 'UNKNOWN_VISIBILITY',
        self::PBPUBLIC => 'PUBLIC',
        self::PBPRIVATE => 'PRIVATE',
        self::INTERNAL => 'INTERNAL',
    ];

    public static function name($value)
    {
        if (!isset(self::$valueToName[$value])) {
            throw new UnexpectedValueException(sprintf(
                    'Enum %s has no name defined for value %s', __CLASS__, $value));
        }
        return self::$valueToName[$value];
    }


    public static function value($name)
    {
        $const = __CLASS__ . '::' . strtoupper($name);
        if (!defined($const)) {
            $pbconst =  __CLASS__. '::PB' . strtoupper($name);
            if (!defined($pbconst)) {
                throw new UnexpectedValueException(sprintf(
                        'Enum %s has no value defined for name %s', __CLASS__, $name));
            }
            return constant($pbconst);
        }
        return constant($const);
    }
}

// Adding a class alias for backwards compatibility with the previous class name.
class_alias(Visibility::class, \Eolymp\Universe\Plan_Visibility::class);

