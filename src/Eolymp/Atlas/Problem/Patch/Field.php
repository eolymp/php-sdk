<?php
# Generated by the protocol buffer compiler.  DO NOT EDIT!
# source: eolymp/atlas/problem.proto

namespace Eolymp\Atlas\Problem\Patch;

use UnexpectedValueException;

/**
 * Protobuf type <code>eolymp.atlas.Problem.Patch.Field</code>
 */
class Field
{
    /**
     * Generated from protobuf enum <code>UNKNOWN_PATCH = 0;</code>
     */
    const UNKNOWN_PATCH = 0;
    /**
     * Generated from protobuf enum <code>VISIBLE = 1;</code>
     */
    const VISIBLE = 1;
    /**
     * Generated from protobuf enum <code>PRIVATE = 2;</code>
     */
    const PBPRIVATE = 2;
    /**
     * Generated from protobuf enum <code>TOPICS = 3;</code>
     */
    const TOPICS = 3;
    /**
     * Generated from protobuf enum <code>DIFFICULTY = 4;</code>
     */
    const DIFFICULTY = 4;
    /**
     * Generated from protobuf enum <code>ORIGIN = 5;</code>
     */
    const ORIGIN = 5;
    /**
     * Generated from protobuf enum <code>TYPE = 6;</code>
     */
    const TYPE = 6;
    /**
     * Generated from protobuf enum <code>NUMBER = 7;</code>
     */
    const NUMBER = 7;

    private static $valueToName = [
        self::UNKNOWN_PATCH => 'UNKNOWN_PATCH',
        self::VISIBLE => 'VISIBLE',
        self::PBPRIVATE => 'PRIVATE',
        self::TOPICS => 'TOPICS',
        self::DIFFICULTY => 'DIFFICULTY',
        self::ORIGIN => 'ORIGIN',
        self::TYPE => 'TYPE',
        self::NUMBER => 'NUMBER',
    ];

    public static function name($value)
    {
        if (!isset(self::$valueToName[$value])) {
            throw new UnexpectedValueException(sprintf(
                    'Enum %s has no name defined for value %s', __CLASS__, $value));
        }
        return self::$valueToName[$value];
    }


    public static function value($name)
    {
        $const = __CLASS__ . '::' . strtoupper($name);
        if (!defined($const)) {
            $pbconst =  __CLASS__. '::PB' . strtoupper($name);
            if (!defined($pbconst)) {
                throw new UnexpectedValueException(sprintf(
                        'Enum %s has no value defined for name %s', __CLASS__, $name));
            }
            return constant($pbconst);
        }
        return constant($const);
    }
}

// Adding a class alias for backwards compatibility with the previous class name.
class_alias(Field::class, \Eolymp\Atlas\Problem_Patch_Field::class);

