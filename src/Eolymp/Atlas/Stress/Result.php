<?php
# Generated by the protocol buffer compiler.  DO NOT EDIT!
# source: eolymp/atlas/stress.proto

namespace Eolymp\Atlas\Stress;

use Google\Protobuf\Internal\GPBType;
use Google\Protobuf\Internal\RepeatedField;
use Google\Protobuf\Internal\GPBUtil;

/**
 * Result is what one compared solution did on one run
 *
 * Generated from protobuf message <code>eolymp.atlas.Stress.Result</code>
 */
class Result extends \Google\Protobuf\Internal\Message
{
    /**
     * Generated from protobuf field <code>string solution_id = 1;</code>
     */
    protected $solution_id = '';
    /**
     * Generated from protobuf field <code>.eolymp.atlas.Submission.Verdict verdict = 2;</code>
     */
    protected $verdict = 0;
    /**
     * Generated from protobuf field <code>bool unexpected = 3 [(.eolymp.api.mcp_field_desc) = "verdict contradicts the solution\'s type: a CORRECT solution not accepted, or a WRONG_ANSWER/TIMEOUT/OVERFLOW solution failing in a different way"];</code>
     */
    protected $unexpected = false;
    /**
     * Generated from protobuf field <code>string output_url = 10;</code>
     */
    protected $output_url = '';
    /**
     * Generated from protobuf field <code>.eolymp.executor.Stats stats = 20;</code>
     */
    protected $stats = null;
    /**
     * Generated from protobuf field <code>.eolymp.executor.Stats checker_stats = 21;</code>
     */
    protected $checker_stats = null;

    /**
     * Constructor.
     *
     * @param array $data {
     *     Optional. Data for populating the Message object.
     *
     *     @type string $solution_id
     *     @type int $verdict
     *     @type bool $unexpected
     *     @type string $output_url
     *     @type \Eolymp\Executor\Stats $stats
     *     @type \Eolymp\Executor\Stats $checker_stats
     * }
     */
    public function __construct($data = NULL) {
        \GPBMetadata\Eolymp\Atlas\Stress::initOnce();
        parent::__construct($data);
    }

    /**
     * Generated from protobuf field <code>string solution_id = 1;</code>
     * @return string
     */
    public function getSolutionId()
    {
        return $this->solution_id;
    }

    /**
     * Generated from protobuf field <code>string solution_id = 1;</code>
     * @param string $var
     * @return $this
     */
    public function setSolutionId($var)
    {
        GPBUtil::checkString($var, True);
        $this->solution_id = $var;

        return $this;
    }

    /**
     * Generated from protobuf field <code>.eolymp.atlas.Submission.Verdict verdict = 2;</code>
     * @return int
     */
    public function getVerdict()
    {
        return $this->verdict;
    }

    /**
     * Generated from protobuf field <code>.eolymp.atlas.Submission.Verdict verdict = 2;</code>
     * @param int $var
     * @return $this
     */
    public function setVerdict($var)
    {
        GPBUtil::checkEnum($var, \Eolymp\Atlas\Submission\Verdict::class);
        $this->verdict = $var;

        return $this;
    }

    /**
     * Generated from protobuf field <code>bool unexpected = 3 [(.eolymp.api.mcp_field_desc) = "verdict contradicts the solution\'s type: a CORRECT solution not accepted, or a WRONG_ANSWER/TIMEOUT/OVERFLOW solution failing in a different way"];</code>
     * @return bool
     */
    public function getUnexpected()
    {
        return $this->unexpected;
    }

    /**
     * Generated from protobuf field <code>bool unexpected = 3 [(.eolymp.api.mcp_field_desc) = "verdict contradicts the solution\'s type: a CORRECT solution not accepted, or a WRONG_ANSWER/TIMEOUT/OVERFLOW solution failing in a different way"];</code>
     * @param bool $var
     * @return $this
     */
    public function setUnexpected($var)
    {
        GPBUtil::checkBool($var);
        $this->unexpected = $var;

        return $this;
    }

    /**
     * Generated from protobuf field <code>string output_url = 10;</code>
     * @return string
     */
    public function getOutputUrl()
    {
        return $this->output_url;
    }

    /**
     * Generated from protobuf field <code>string output_url = 10;</code>
     * @param string $var
     * @return $this
     */
    public function setOutputUrl($var)
    {
        GPBUtil::checkString($var, True);
        $this->output_url = $var;

        return $this;
    }

    /**
     * Generated from protobuf field <code>.eolymp.executor.Stats stats = 20;</code>
     * @return \Eolymp\Executor\Stats|null
     */
    public function getStats()
    {
        return $this->stats;
    }

    public function hasStats()
    {
        return isset($this->stats);
    }

    public function clearStats()
    {
        unset($this->stats);
    }

    /**
     * Generated from protobuf field <code>.eolymp.executor.Stats stats = 20;</code>
     * @param \Eolymp\Executor\Stats $var
     * @return $this
     */
    public function setStats($var)
    {
        GPBUtil::checkMessage($var, \Eolymp\Executor\Stats::class);
        $this->stats = $var;

        return $this;
    }

    /**
     * Generated from protobuf field <code>.eolymp.executor.Stats checker_stats = 21;</code>
     * @return \Eolymp\Executor\Stats|null
     */
    public function getCheckerStats()
    {
        return $this->checker_stats;
    }

    public function hasCheckerStats()
    {
        return isset($this->checker_stats);
    }

    public function clearCheckerStats()
    {
        unset($this->checker_stats);
    }

    /**
     * Generated from protobuf field <code>.eolymp.executor.Stats checker_stats = 21;</code>
     * @param \Eolymp\Executor\Stats $var
     * @return $this
     */
    public function setCheckerStats($var)
    {
        GPBUtil::checkMessage($var, \Eolymp\Executor\Stats::class);
        $this->checker_stats = $var;

        return $this;
    }

}

// Adding a class alias for backwards compatibility with the previous class name.
class_alias(Result::class, \Eolymp\Atlas\Stress_Result::class);

